...
granular_supra_ee0 = tf.Variable(tf.zeros(shape=(exc, exc), dtype=tf.float32))
granular_supra_ei0 = tf.Variable(tf.zeros(shape=(inter, inter), dtype=tf.float32))

global1 = tf.Variable(0)
global2 = tf.Variable(0)

# queue  = queue = tf.FIFOQueue(1, shapes=(), shared_name="Que", dtypes=tf.int32)
...
with tf.device("/job:worker/task:0"):
    ...
    connect = tf.group(granular_supra_ee0.assign(granular_supra_ee), granular_supra_ei0.assign(granular_supra_ei))
...
with tf.device("/job:worker/task:1"):
    ...
    supra_pyramidal = tf.add(tf.add(supra_ee,supra_ie),tf.reshape(granular_supra_ee0,(1,n_exc)))
    supra_basket = tf.add(tf.add(supra_ii,supra_ei),tf.reshape(granular_supra_ei0,(1,n_inter)))
...
if FLAGS.job_name == "ps":
    ...
    with tf.Session(server.target) as sess:
        if is_chief:
            ...
            print("chief initialized")
        else:
            sess.run(tf.global_variables_initializer())
            while len(sess.run(tf.report_uninitialized_variables())) > 0:
                print("worker:%d waiting for initialization" % FLAGS.task_index)
            print("worker:%d ready"% FLAGS.task_index)
        ...
        if FLAGS.task_index == 0:
            ...
            for i in range(20000):
                ...
                state_granular_pyramidal, delay_granular_pyramidal, state_granular_basket, delay_granular_basket,\
                Input_granular_pyramidal2, Input_granular_basket2,_,g1 = \
                    sess.run([granular_pyramidal_nextstate, granular_pyramidal_synstate, granular_basket_nextstate,
                              granular_basket_synstate,granular_pyramidal, granular_basket, connect,global1.assign_add(1)],
                             feed_dict={granular_pyramidal_input: Input_granular_pyramidal1,
                                        ...
                                        granular_basket_state: state_granular_basket,
                                        granular_basket_delay: delay_granular_basket})
                ...
                while sess.run(global2) != g1:
                    time.sleep(0.0005)
                if i%100 == 0:
                    print('step %d'%i)
            np.savetxt('GP.csv', P_granular_pyramidal)
            np.savetxt('GB.csv', P_granular_basket)
            print(time.time()-t)
        ...
        if FLAGS.task_index == 1:
            ...
            t = time.time()
            for i in range(20000):
                state_supra_pyramidal, delay_supra_pyramidal, state_supra_basket, delay_supra_basket,Input_supra_pyramidal2, Input_supra_basket2, g2 = \
                    sess.run([supra_pyramidal_nextstate, supra_pyramidal_synstate, supra_basket_nextstate,
                              supra_basket_synstate, supra_pyramidal, supra_basket, global2.assign_add(1)],
                             feed_dict={supra_pyramidal_input: Input_supra_pyramidal,
                                        supra_pyramidal_state: state_supra_pyramidal,
                ...
                while sess.run(global1) != g2:
                    time.sleep(0.0005)
                # sess.run(queue.dequeue())
                if i % 100 == 0:
                    print('step %d' % i)
            np.savetxt('SP.csv', P_supra_pyramidal)
            np.savetxt('SB.csv', P_supra_basket)
            print(time.time()-t)
